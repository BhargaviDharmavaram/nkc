...
const ExpensesContainer = () => {
    ...
    useEffect(() => {
        const fetchExpenses = async () => {
            try {
                const expensesResponse = await axios.get('https://nkc-6nv4.onrender.com/api/get-expenses');
                setExpenses(expensesResponse.data.expenses);
                console.log(expensesResponse.data, 'expenses');
                const usersResponse = await axios.get('https://nkc-6nv4.onrender.com/api/get-users');
                setUsers(usersResponse.data);
                const categoriesResponse = await axios.get('https://nkc-6nv4.onrender.com/api/get-categories');
    ...
    const addExpense = async (newExpenseData) => {
        ...
        try {
            const response = await axios.post('https://nkc-6nv4.onrender.com/api/add-expenses', newExpenseData);
            Swal.fire({
                icon: 'success',
    ...
    const removeExpense = async (expenseId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`https://nkc-6nv4.onrender.com/api/delete-expenses/${expenseId}`);
                    Swal.fire({
                        icon: 'success',
                        title: response.data.message
                    });
    ...
    const handleEditSubmit = async (updatedExpense) => {
        try {
            const response = await axios.put(`https://nkc-6nv4.onrender.com/api/update-expenses/${updatedExpense._id}`, updatedExpense);
            Swal.fire({
                icon: 'success',
                title: response.data.message
            });
